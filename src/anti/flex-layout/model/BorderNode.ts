...
class BorderNode extends Node implements IDropTarget {
  ...
  layoutBorderInner(inner: Rect) {
    ...
    const split1 = location.split(inner, this.adjustedSize + this.model.getSplitterSize()) // split off tab contents
    const split2 = location.reflect().split(split1.start, this.model.getSplitterSize()) // split contents into content and splitter
    ...
    if (this.getSelected() === -1) return inner
