...
import BorderTabSet from 'components/flex-layout/view/BorderTabSet'
...
export class Layout extends React.Component<ILayoutProps, any> {
  ...
  /** @hidden @internal */
  updateRect = () => {
    const domRect = (this.layoutRef.current as HTMLDivElement).getBoundingClientRect()
  ...
  onDragMove = (event: React.MouseEvent<Element>) => {
    ...
    const clientRect = (this.layoutRef.current as HTMLDivElement).getBoundingClientRect()
    const pos = {
  ...
  hideEdges = (rootdiv: HTMLElement) => {
    if (this.model!.isEnableEdgeDock()) {
      try {
        ...
        // console.log('There is error in 'hideEdges')
      }
    }
  }
